...
declare -A TEST_CONFIGS=(
    ["basic"]="basic github.com/test/basic-service"
    ["intermediate"]="intermediate github.com/test/intermediate-service"
    ["advanced"]="advanced github.com/test/advanced-service"
    ["enterprise"]="enterprise github.com/test/enterprise-service"
)
...
test_tier_generation() {
    local tier=$1
    local config=$2
    local tier_name module_path
    read -r tier_name module_path <<< "$config"
...
main() {
    ...
    # Test each tier
    for tier in "${!TEST_CONFIGS[@]}"; do
        test_tier_generation "$tier" "${TEST_CONFIGS[$tier]}"
    done
